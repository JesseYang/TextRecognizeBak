...
# print(font_paths)
print(font_paths== ['Optima', 'verdana', 'helvetica', 'courier', 'arials', 'Caslons', 'AdobeGaramond', 'frutiger', 'din', 'adele', 'BookmanOldStyle', 'futura', 'Bodonis', 'new_roman', 'Consola'])
...
# print('Consola', 'Optima', 'arials', 'Caslons', 'futura', 'AdobeGaramond', 'Bodonis', 'new_roman', 'helvetica', 'adele', 'BookmanOldStyle', 'courier', 'din', 'verdana', 'frutiger')
 #['Optima', 'verdana', 'helvetica', 'courier', 'arials', 'Caslons', 'AdobeGaramond', 'frutiger', 'din', 'adele', 'BookmanOldStyle', 'futura', 'Bodonis', 'new_roman', 'Consola']
    #['Consola', 'Optima', 'arials', 'Caslons', 'futura', 'AdobeGaramond', 'Bodonis', 'new_roman', 'helvetica', 'adele', 'BookmanOldStyle', 'courier', 'din', 'verdana', 'frutiger']
    #[0.05,  0.05, 0.05, 0.1,  0.05,  0.1,  0.1,  0.1,  0.05,  0.05, 0.1, 0.05, 0.05,  0.05, 0.05]

    #[0.005, 0.04, 0.03, 0.05, 0.05, 0.145, 0.11, 0.05, 0.05, 0.05, 0.12, 0.005, 0.145, 0.1, 0.05]
    ...
    with open(article_path, errors='ignore') as article:
        for line in article:
            ...
            for subline in sublines:
                ...
                if index == 0 and random.uniform(0, 1) > 0.75:
                    text = text.upper()
                elif index == 1 and random.uniform(0, 1) > 0.65:
                    text = text.title()
                elif index == 2 and random.uniform(0, 1) > 0.65:
                    text = text.capitalize()
                elif index == 3 and random.uniform(0, 1) > 0.55:
                    text == text.lower()
                else:
                    pass
                # print(text)
                # randomly pick a font and height
                font_pro = np.random.choice(font_paths, 1, p=[0.005, 0.04, 0.03, 0.05, 0.05, 0.145, 0.11, 0.05, 0.05, 0.05, 0.12, 0.005, 0.145, 0.1, 0.05])[0]
                ...
                if random.uniform(0, 1) >= 0.5:
                    canvas = canvas + random.uniform(100, 224)
