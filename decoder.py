...
class BeamCTCDecoder(Decoder):
    ...
    def convert_to_strings(self, out, seq_len):
        ...
        for p, paths in enumerate(out):
            ...
            for u, utt in enumerate(paths):
                utterances.append(''.join(map(lambda x: self.int_to_char[x], utt[0:seq_len[p][u]] if seq_len[p][u] > 0 else '')))
    ...
    def decode(self, probs, sizes=None):
        ...
        new_probs = probs.transpose(0,1)
        new_probs = new_probs.view((new_probs.size()[0], 1, new_probs.size()[1]))
        out, conf, seq_lens, offsets, char_probs = self._decoder.decode(new_probs.cpu(), sizes)
